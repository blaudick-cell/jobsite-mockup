...
| key | seed | shape |
|---|---|---|
| `projects` | `PROJECTS_SEED` | `{ id, code, name, gc, address, startDate, status, material: [], haulType: 'NEEDS'\|'HAS', contactName, contactPhone, contactEmail, volumeCYNeeded: number\|null, volumeCYKnown: boolean, pickupLocation: string, dumpLocation: string }` — v16: 3 real Denver-area projects (`RR` 5800 Federal active 100k CY NEEDS, `RG` 1105 Rogers Street upcoming 200 CY HAS, `GE` Geos Neighborhood upcoming unknown-CY HAS). v17: each project carries a `pickupLocation` + `dumpLocation` address pair. For NEEDS, dump = project's own address + pickup = a 5-30 min counterpart; for HAS, vice versa. The Live Operations Map draws an OSRM-fetched driving polyline between them. `haulType` is the explicit haul-direction field. `volumeCYNeeded` is nullable when the volume isn't known; `volumeCYKnown: false` signals the UI to render "—" instead of "0". |
...
| v16 → v17 | Added `db.routes` collection — plain object keyed by `<pickupAddress>\|<dumpAddress>`, value `{ coords: [[lng, lat], ...], distanceMeters, durationSeconds }`. Backs the OSRM driving-route layer in `LiveOperationsTileMap`. Each project carries new `pickupLocation` + `dumpLocation` address strings (Robert's 3 projects each have a real 5-30 min counterpart). Migration backfills `routes: {}` if missing, merges `GEOCODE_SEED` to add pre-seeded coords for the 3 new counterpart addresses (Wheat Ridge / Lakewood / Wheat Ridge), and ensures stored projects pick up the new `pickupLocation`/`dumpLocation` fields via defaults-first spread. Real geocoding via Nominatim (`https://nominatim.openstreetmap.org/search`) fires on mount only for missing addresses; OSRM (`https://router.project-osrm.org/route/v1/driving/...?geometries=geojson&overview=full`) fires only for routes not in the cache. Both wrap try/catch and degrade to straight-line rendering on failure. Cache is keyed on raw address pair so it survives across mounts. |

`DB_SCHEMA_VERSION = 17`. See [[jse-ship-a-feature]] § Schema migration for how to bump.
