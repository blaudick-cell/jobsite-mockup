...
**`DB_SCHEMA_VERSION` is currently 13.** Migration cascade in `hydrateDb` — every step forward-only, idempotent, wrapped in try/catch, falls back to seed on failure. See [[jse-data-model]] § Schema version log for the full history.
